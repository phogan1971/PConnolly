  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          <Link href="/stock" className="flex items-center gap-2 text-lg font-semibold text-slate-900 hover:text-slate-900">
            ...
            <span>{dealership?.tradingName ?? dealership?.name ?? "DealerOps"}</span>
          </Link>
          <nav className="flex items-center gap-5 text-sm text-slate-700">
            <Link href="/stock">Stock</Link>
            <a href={dealership?.phone ? `tel:${dealership.phone.replace(/\s+/g, "")}` : "#"} className="hidden sm:inline">
              {dealership?.phone}
            </a>
            <Link href="/admin" className="rounded-md bg-slate-100 px-3 py-1.5 text-slate-700 hover:bg-slate-200 hover:text-slate-900">
              Staff login
            </Link>
          </nav>
        </div>
      ...
      <footer className="mt-16 border-t border-slate-200 py-6 text-center text-xs text-slate-500">
        © {new Date().getFullYear()} {dealership?.tradingName ?? dealership?.name ?? "DealerOps"}. Powered by DealerOps.
      </footer>
    </div>
  );
}
